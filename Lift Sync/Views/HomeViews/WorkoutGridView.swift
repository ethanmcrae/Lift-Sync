...
struct WorkoutGridView: View {
    ...
    @Binding var isAddNewCategorySelected: Bool
    ...
    var body: some View {
        ZStack {
                ...
                if !selectedCategory.isEmpty {
                    ...
                    NavigationLink(destination: NewWorkoutFormView(isPresenting: .constant(false), qrCode: nil, onComplete: { newWorkout in
                        ...
                        .popover(isPresented: TutorialManager.isShowingPopover(TutorialManager.Tutorial.home, currentStep: $homeTutorialStep, expected: 6)) {
                            TutorialHomePopup(text: "Create your first exercise here", step: $homeTutorialStep, tutorial: TutorialManager.Tutorial.home)
                        }
                    }
            ...
            if selectedCategory.isEmpty && !isAddNewCategorySelected {
                Text("Select a workout category above...")
                    .font(isiPad ? .title : .title3)
                    .popover(isPresented: TutorialManager.isShowingPopover(.home, currentStep: $homeTutorialStep, expected: 4)) {
                        TutorialHomePopup(text: "Let's create some exercises so they can appear here", step: $homeTutorialStep, tutorial: .home)
                    }
                Spacer()
                    .frame(height: 300)
            }
...
#Preview {
    ...
    @State var isAddNewCategorySelected = false
    ...
    return ZStack {
        ...
        VStack {
            Spacer()
                .frame(maxHeight: .infinity)
            WorkoutGridView(selectedCategory: $selectedCategory, onDisappear: {}, homeTutorialStep: $homeTutorialStep, isAddNewCategorySelected: $isAddNewCategorySelected)
                .environmentObject(workoutManager)
